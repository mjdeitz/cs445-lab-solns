//Use the module pattern to create a shopping cart
...
const shoppingCart = (function () {
    ...
    return {
        upsertItem: function (item) {
            const index = basket.findIndex(basketItem => basketItem.id === item.id);//findIndex is existing method provided by Array
            //if exists index will return index of item or will return -1
            if (index > -1) {
                basket[index] = item;
            } else {
                basket.push(item);
